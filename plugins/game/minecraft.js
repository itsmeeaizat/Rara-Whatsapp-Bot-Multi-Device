...
let thumbMC = null;
try {
  const p = path.join(process.cwd(), "assets", "images", "rara-minecraft.jpg");
  if (fs.existsSync(p)) thumbMC = fs.readFileSync(p);
} catch (e) {}

function ctx() {
  const sId = config.saluran?.id || "example@newsletter";
  const sName = config.saluran?.name || config.bot?.name || "Rara-AI";
  return {
