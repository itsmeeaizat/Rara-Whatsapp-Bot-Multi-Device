...
try {
  ...
  if (fs.existsSync(path.join(assetsPath, "rara.jpg"))) {
    ...
    thumbDay = fs.readFileSync(path.join(assetsPath, "rara.jpg"));
  }
  if (fs.existsSync(path.join(assetsPath, "rara-winner.jpg"))) {
    thumbWin = fs.readFileSync(path.join(assetsPath, "rara-winner.jpg"));
  }
} catch (e) {
...
function getRoleDescription(role, prefix = ".") {
  const descriptions = {
    werewolf:
      ...
      `┃ ◦ Skill: Bunuh 1 player tiap malam\n` +
      `┃ ◦ Aksi: Malam hari\n` +
      `╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈❀\n\n\n` +
      `> Di malam hari, ketik:\n` +
  ...
  if (!action || !commands[action]) {
    return m.reply(
      `🐺 *WEREWOLF GAME*\n\n` +
        ...
        `┃ ◦ \`${prefix}ww join\` - Gabung\n` +
        `┃ ◦ ▶️ \`${prefix}ww start\` - Mulai (host)\n` +
        `┃ ◦ \`${prefix}ww vote <no>\` - Vote\n` +
        `┃ ◦ \`${prefix}ww player\` - List player\n` +
        `┃ ◦ \`${prefix}ww exit\` - Keluar\n` +
        `┃ ◦ \`${prefix}ww delete\` - Hapus room\n` +
        `╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈❀\n\n\n` +
        `╭┈❀ *ROLES*\n` +
