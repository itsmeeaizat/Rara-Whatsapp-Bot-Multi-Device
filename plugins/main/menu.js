...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  try {
    imageBuffer = fs.readFileSync(botConfig.assets["rara"])
    thumbBuffer = fs.readFileSync(botConfig.assets["rara2"])
  } catch (e) {
                  ...
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      ...
                      tap_target_configuration: {
                        ...
                        canonical_url: "https://aizat.site",
      ...
      case 3:
        const content = {
          buttonsMessage: {
            ...
            locationMessage: {
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["rara"])).resize(300, 170).toBuffer(),
              name: config.bot.name,
...
╰┈┈┈┈┈┈┈┈`,
      ...
      case 4: {
        const thumbnail = await sharp(fs.readFileSync(config.assets["rara"])).resize(300, 300).toBuffer()
        const qvideo = {
          key: {
            fromMe: false,
            participant: m.sender
        ...
        const media4 = await prepareWAMessageMedia({
          video: fs.readFileSync(config.assets["rara-mp4"]),
          gifPlayback: true
        }, { upload: sock.waUploadToServer });
                ...
                nativeFlowMessage: {
                  messageParamsJson: JSON.stringify({
                    ...
                    tap_target_configuration: {
                      title: " X ",
                      description: "bomboclard",
                      canonical_url: "https://aizat-studio.my.id",
                      domain: "aizat-studio.my.id",
                      button_index: 0,
                    },
                  }),
                  buttons: [
