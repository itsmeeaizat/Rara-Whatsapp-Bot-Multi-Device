...
function getContextInfo(botConfig, m, thumbBuffer) {
  const saluranId = botConfig.saluran?.id || "example@newsletter";
  const saluranName =
  ...
  try {
    switch (allmenuVariant) {
      ...
      case 5: {
        ...
        const thumbnail = await _sharp(fs.readFileSync(config.assets["rara"])).resize(300, 300).toBuffer()
        const qOrder = {
        ...
        const media4 = await prepareWAMessageMedia({ video: fs.readFileSync(config.assets["rara-mp4"]), gifPlayback: true }, { upload: sock.waUploadToServer });
            ...
            message: {
              ...
              interactiveMessage: {
                ...
                contextInfo: {
                  ...
                  forwardingScore: 9,
                  forwardedNewsletterMessageInfo: {
                    newsletterJid: config.saluran?.id || "example@newsletter",
                    newsletterName: config.saluran?.name || config.bot?.name || "Rara-AI",
                    serverMessageId: 127,
                  },
                },
                nativeFlowMessage: {
                  messageParamsJson: JSON.stringify({
                    ...
                    tap_target_configuration: { title: " X ", description: "bomboclard", canonical_url: "https://aizat.site", domain: "shop.example.com", button_index: 0 },
                  }),
                  buttons: [
    ...
    if (audioEnabled) {
      const audioUrl = botConfig.assets["rara-mp3"];
      ...
      try {
        ...
        switch (audioVariant) {
          ...
          case 3: {
            ...
            await sock.sendMessage(m.chat, {
              audio: fs.readFileSync(config.assets["rara-mp3"]),
              mimetype: "audio/mpeg",
              ptt: false,
            }, { quoted: qtext });
            break;
              ...
              message: {
                orderMessage: {
                  ...
                  contextInfo: {
                    ...
                    forwardingScore: 9,
                    forwardedNewsletterMessageInfo: {
                      newsletterJid: "example@newsletter",
                      newsletterName: "Rara Bot",
                      serverMessageId: 127,
                    },
                  },
                },
