...
` +
    ...
    if (globalMode === "on") {
      ...
      if (charKey === "custom" && customLogic) {
        ...
        if (existingGlobal.instruction) {
          ...
          return m.reply(
            `❌ Belum ada persona global yang diset!\n\n> .autoai global on --raramode=furina\n> .autoai global on --raramode=custom --logic=...`,
          );
        }
      } else {
        const charList = [
  ...
  if (!mode || !["on", "off"].includes(mode)) {
    ...
    txt += `*Penggunaan:*\n`;
    txt += `> .autoai on --raramode=<karakter|custom> --type=<text|voice> --mode=<onlychat|assistant>\n`;
    txt += `> .autoai off\n`;
    txt += `> .autoai tambahpersona nama | logic\n`;
    txt += `> .autoai hapuspersona nama\n`;
    ...
    txt += `*Contoh:*\n`;
    txt += `> .autoai on --raramode=furina --type=text\n`;
    txt += `> .autoai on --raramode=custom --logic=kamu adalah nexa ai\n`;
    txt += `> .autoai tambahpersona nexa | kamu adalah nexa ai\n`;
    txt += `> .autoai global on --raramode=furina`;
    return m.reply(txt);
  }
  ...
  if (!charKey) {
    ...
    return m.reply(
      `❌ Karakter tidak valid!\n\n> Karakter tersedia: ${charList}\n\n> Contoh: .autoai on --raramode=furina --type=voice\n> Custom: .autoai on --raramode=custom --logic=kamu adalah nexa ai`,
    );
  }
  ...
  if (charKey === "custom") {
    if (!customLogic) {
      return m.reply(
        `❌ Mode custom membutuhkan --logic!\n\n> Contoh: .autoai on --raramode=custom --logic=kamu adalah nexa ai, ...`,
      );
    }
...
async function generateVoiceResponse(text, sock, chatId, quotedMsg) {
  ...
  try {
    ...
    const apiUrl = `https://api.example.com/api/crikk?apikey=${config.APIkey.firefly}&text=${encodeURIComponent(text)}&voice=id-ID-ArdiNeural`;
