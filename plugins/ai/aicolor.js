...
const pluginConfig = { name: 'aicolor', alias: ['colorpalette', 'palet'], category: 'ai', description: 'Ekstrak palet warna dari gambar', usage: '.aicolor (reply gambar)', example: '.aicolor', isOwner: false, isPremium: true, isGroup: false, isPrivate: false, cooldown: 10, energi: 5, isEnabled: true }
async function handler(m, { sock }) {
    ...
    try {
        ...
        const res = await axios.post(aiConfig.gemini.baseURL + '/models/' + aiConfig.gemini.visionModel + ':generateContent?key=' + aiConfig.gemini.key, {
            contents: [{ parts: [{ text: 'Ekstrak 5 warna dominan dari gambar. Format: 1. #HEX - nama warna - persentase penggunaan.' }, { inline_data: { mime_type: mime, data: base64 } }] }]
        })
        const result = res.data.candidates?.[0]?.content?.parts?.[0]?.text || 'Tidak ada respons'
        return m.reply('╭┈❀ *AI COLOR PALETTE*\n┃\n┃ ◦ ' + result + '\n┃\n╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈❀')
