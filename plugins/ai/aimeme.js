...
const pluginConfig = { name: 'aimeme', alias: ['aimemegen'], category: 'ai', description: 'Generate caption meme dari gambar', usage: '.aimeme (reply gambar)', example: '.aimeme', isOwner: false, isPremium: true, isGroup: false, isPrivate: false, cooldown: 10, energi: 5, isEnabled: true }
async function handler(m, { sock }) {
    ...
    try {
        ...
        const res = await axios.post(aiConfig.gemini.baseURL + '/models/' + aiConfig.gemini.visionModel + ':generateContent?key=' + aiConfig.gemini.key, {
            contents: [{ parts: [{ text: 'Buat caption meme yang lucu untuk gambar ini dalam bahasa Indonesia. Format: teks atas | teks bawah.' }, { inline_data: { mime_type: mime, data: base64 } }] }]
        })
        const result = res.data.candidates?.[0]?.content?.parts?.[0]?.text || 'Tidak ada respons'
        return m.reply('╭┈❀ *AI MEME GENERATOR*\n┃\n┃ ◦ ' + result + '\n┃\n╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈❀')
