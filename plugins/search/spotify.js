...
async function handler(m, { sock, text }) {
  ...
  try {
    const res = await axios.get(`https://api.cuki.biz.id/api/search/spotify?apikey=&query=${encodeURIComponent(text)}&limit=5`);
    const data = res.data;
    ...
    if (thumbnailBuffer) {
      const content = {
        buttonsMessage: {
          ...
          footerText: '🚀 Rara Multi Device - Spotify Search',
          headerType: 6,
        },
      };
