...
import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./rara-game-data.js";
import { getDatabase } from "./rara-database.js";
import { addExpWithLevelCheck } from "./rara-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./rara-context.js";
import config from "../../config.js";
import fs from "fs";
import sharp from "sharp";
let fetchBuffer;
try {
  fetchBuffer = (await import("./rara-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["rara2"])).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
