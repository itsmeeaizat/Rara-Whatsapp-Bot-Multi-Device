...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "RARAAI" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Rara-AI";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        m.chat,
        {
          ...
          jpegThumbnail: srtImage ? await sharp(srtImage).resize(300, 300).toBuffer() : await sharp(getAssetBuffer("rara2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("rara3");
      return sock.sendMessage(
        m.chat,
        {
          video: getAssetBuffer("rara-mp4"),
          caption: text,
          gifPlayback: true,
          contextInfo: {
            ...contextInfo,
      ...
      const thumbnail = srtImage || getAssetBuffer("rara");
      ...
      const thumbnailBuf = srtImage || getAssetBuffer("rara");
      const fakeOrder = {
        key: {
          participant: "0@s.whatsapp.net",
          remoteJid: m.chat,
        ...
        message: {
          orderMessage: {
            ...
            status: 1,
            surface: 1,
            message: config.bot?.name || "Rara-AI",
