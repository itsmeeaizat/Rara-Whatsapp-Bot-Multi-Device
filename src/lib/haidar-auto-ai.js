...
footer: Powered by Rara AI (opsional)
[/RICH:CODE]
...
[RICH:LIST]
title: Info Bot
rows: Nama | Rara AI;; Versi | 2.4.5;; Developer | haidar
footer: © Rara AI (opsional)
[/RICH:LIST]

5. STICKER (saat user minta sticker, atau untuk ekspresi emosi):
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
packname: Rara AI (opsional)
  ...
  try {
    ...
    if (rich.type === "STICKER") {
      ...
      await sock.sendImageAsSticker(jid, stickerInput, quoted, {
        packname: packname || config.bot?.name || "Rara AI",
  ...
  try {
    ...
    if (autoai.responseType === "voice") {
      try {
        ...
        const apiUrl = `https://api.example.com/api/crikk?apikey=${config.APIkey.firefly}&text=${encodeURIComponent(cleanResponse.substring(0, 500))}&voice=id-ID-ArdiNeural`;
