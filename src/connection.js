...
async function startConnection(options = {}) {
  ...
  const sock = makeWASocket({
    ...
    // FIX: Menggunakan Ubuntu Chrome untuk menghindari pemblokiran Pairing Code oleh WA
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      // FIX: Menambahkan timeout lebih stabil (3 detik)
      await new Promise((resolve) => setTimeout(resolve, 3000));
      
      // FIX: Menghapus argumen kedua ("RARAMD") agar tidak bad request
      let code = await sock.requestPairingCode(phoneNumber, "RARAMD");
      
      // FIX: Format Pairing Code dengan tanda "-" supaya estetik
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `${config.bot?.name || "Rara-AI"} (${n || "?"}) · WA v${version.join(".")}`,
      );
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          ...
          const saluranId =
            config.saluran?.id || "example@newsletter";
          const saluranName =
